...
唤出窗口时主动重扫一次；后台每 20 分钟静默刷新。主面板快捷键：`/` 聚焦搜索（按名称/备忘过滤），`Esc` 隐藏到托盘。卡片在「全部」视图下可拖拽自由重排（持久化）；卡片详情里可把一个项目「设为主攻」，固定占据左侧大卡位。警示 pill 悬停可点 × 消音，状态变化后自动复出。
...
- **GitHub Token + 用户名**：「测试连接」验证 token 有效且登录名一致；配置后拉取 owner 是自己的仓库的开放 issues/PR（10 分钟缓存）；fork 的上游仓库自然跳过
...
- `prefs.json`：主攻图钉、卡片自由位序、警示消音、窗口尺寸位置
